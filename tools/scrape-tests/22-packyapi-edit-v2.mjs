#!/usr/bin/env node
/**
 * Test 22: packyapi /v1/images/edits — v2 with `image[]` field + File ctor
 * (fixes "multipart: NextPart: EOF" from v1).
 */
...
if (!INPUT) { console.error('no input png'); process.exit(1); }
...
fd.append('n', '1');
const file = new File([buf], 'input.png', { type: 'image/png' });
fd.append('image[]', file);

console.log(`[edit v2] POST ${BASE}/v1/images/edits  model=${MODEL} size=${SIZE} quality=${QUALITY}`);
const t0 = Date.now();
...
console.log(`[edit v2] HTTP ${res.status} ${ct} ${elapsed}ms bytes=${txt.length}`);
...
if (!res.ok) {
  ...
  writeFileSync(join(OUT, `packyapi-edit-v2-${STAMP}.json`), JSON.stringify(log, null, 2));
  process.exit(1);
}
...
writeFileSync(join(OUT, `packyapi-edit-v2-${STAMP}.json`), JSON.stringify(log, null, 2));
console.log(`[done] log → testoutput/packyapi-edit-v2-${STAMP}.json`);
