 ...
 *  Only Flash/Pro support multi-res; gemini-2.5-flash-image has fixed resolution. */
function geminiSize(resolution: string, model: string): string | undefined {
  if (model === MODEL_MAP['Standard']) return undefined; // gemini-2.5-flash-image: fixed res, no imageSize param
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt: rawPrompt, nodeId, type, settings = {}, referenceImages: incomingRefImages, referenceUrls = [], thoughtSignature: incomingThoughtSig, useStreaming = false } = body;
    // Strip [API: ...] tag if client-side strip didn't fire (safety net)
    ...
    if (type === 'model-creation') {
      ...
      const safetyThresh    = (settings.safetyThreshold as SafetyThreshold | undefined) ?? 'BLOCK_NONE';
      const aspectRatio     = modelCreationAspectRatio(prompt);
      const imageSize       = geminiSize((settings.imageSize as string | undefined) ?? (settings.resolution as string | undefined) ?? '1K', model);
      ...
      const geminiConfig = {
        ...
        imageConfig: { aspectRatio, ...(imageSize !== undefined ? { imageSize } : {}), mediaResolution: 'media_resolution_high' },
    ...
    // Merge canvas-connected reference images (with real names) with tag-matched ones.
    // referenceImages carries {url, name} from the client; referenceUrls is legacy fallback.
    const incomingRefs: Array<{ url: string; name: string }> = incomingRefImages
      ?? referenceUrls.map(url => ({ url, name: 'canvas-reference' }));

    const explicitRefs = incomingRefs
      .filter(ref => !matchedImages.find(m => m.url === ref.url))
      .map(ref => ({ url: ref.url, name: ref.name, matchedTags: [] as string[] }));
    const allImages = [...explicitRefs, ...matchedImages].slice(0, 14);

    const aspectRatio = (settings.aspectRatio as string | undefined) ?? '4:5';

    console.log(`[generate] ── SLIDE GENERATION ──`);
    console.log(`[generate] provider: gemini | nodeId: ${nodeId}`);
    console.log(`[generate] aspectRatio: ${aspectRatio}`);
    console.log(`[generate] referenceImages (${allImages.length} total):`);
    allImages.forEach((img, i) => {
      const src = explicitRefs.find(r => r.url === img.url) ? 'canvas-connected' : 'tag-matched';
      console.log(`[generate]   [${i}] "${img.name}" (${src}) tags=[${img.matchedTags.join(',')}]`);
    });
    if (matchedImages.length === 0 && allImages.length === 0) {
      console.log(`[generate]   (none)`);
    }
    const imageSize     = geminiSize((settings.resolution as string | undefined) ?? '1K', model);
    const textPrompt    = buildSlidePrompt(prompt, settings, allImages, aspectRatio);
    console.log(`[generate] assembled textPrompt (first 500): "${textPrompt.slice(0, 500)}"`);
    ...
    const geminiConfig = {
      ...
      responseModalities: ['TEXT', 'IMAGE'],
      thinkingConfig: { includeThoughts, ...(incomingThoughtSig ? { thoughtSignature: incomingThoughtSig } : {}) },
      imageConfig: { aspectRatio, ...(imageSize !== undefined ? { imageSize } : {}), mediaResolution: mediaRes },
      safetySettings: buildSafetySettings(safetyThresh),
      ...(searchTools ? { tools: searchTools } : {}),
    };
...
function classifyRef(name: string): 'model' | 'setting' | 'product' | 'generic' {
  const n = name.toLowerCase();
  if (n.includes('model') || n.includes('person') || n.includes('talent') || n.includes('subject')) return 'model';
  if (n.includes('setting') || n.includes('background') || n.includes('environment') || n.includes('scene') || n.includes('location')) return 'setting';
  if (n.includes('product') || n.includes('item') || n.includes('prop') || n.includes('earbud') || n.includes('watch') || n.includes('phone') || n.includes('bag') || n.includes('shoe') || n.includes('accessory') || n.includes('device') || n.includes('airpod') || n.includes('case') || n.includes('bottle') || n.includes('perfume') || n.includes('cosmetic') || n.includes('brand')) return 'product';
  return 'generic';
}

function buildRefDirectives(refs: Array<{ name: string; matchedTags: string[] }>): string {
  if (!refs.length) return '';
  const lines: string[] = ['REFERENCE IMAGES — follow these exactly, they are not suggestions:'];
  for (const ref of refs) {
    const role = classifyRef(ref.name);
    if (role === 'model') {
      lines.push(`• Image "${ref.name}" = HUMAN SUBJECT REFERENCE. Match this person's face structure, skin tone, eye shape, hair color/texture, and body type exactly. Do not alter or reinterpret the model's appearance. This is the identity anchor — preserve it precisely.`);
    } else if (role === 'setting') {
      lines.push(`• Image "${ref.name}" = ENVIRONMENT/SETTING REFERENCE. Recreate this location, lighting direction, color temperature, and atmosphere faithfully. The environment in the output must visually match this reference — same location type, same light quality.`);
    } else if (role === 'product') {
      lines.push(`• Image "${ref.name}" = PRODUCT REFERENCE. Replicate this product's exact shape, color, material finish, branding, and proportions with zero creative deviation. This is a real commercial product — no stylization, no substitution.`);
    } else {
      lines.push(`• Image "${ref.name}" = VISUAL REFERENCE. Use it as a precise anchor — reproduce its key visual elements (color, form, subject, lighting) faithfully in the output.`);
    }
  }
  return lines.join('\n') + '\n\n';
}
...
function buildSlidePrompt(
  ...
  const refBlock = buildRefDirectives(refs);
  const ratioHint =
  ...
  return `${refBlock}${prompt}. ${ratioHint}${neg ? ` AVOID: ${neg}.` : ''}${qualityTail}`;
}
...
Each model must be visually consistent across their two panels. Ultra high quality, sharp details, professional fashion photography.`;
  ...
  return `Create a single full-body portrait in 16:9 landscape format. Show the model standing upright, full figure from head to toe, centered in frame.
Model: ${description}.
Style: ${style}. Lighting: ${light}. Background: ${bg}.
Ultra high quality, sharp details, professional fashion photography. No panels, no composites — one single image.`;
}
