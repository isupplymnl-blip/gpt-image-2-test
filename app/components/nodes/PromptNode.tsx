...
import { normalizePrompt } from '../../lib/chatToolBridge';
...
// Parses [API:openai|gemini,model=...,...] — dispatches by tag's first token (or model= value).
// Sub-providers: pudding → Gemini path, pudding-openai → OpenAI path (handled at generation time, not here).
...
export default function PromptNode({ id, data }: NodeProps<PromptNodeData>) {
  ...
  const [prompt, setPrompt] = useState(() =>
    normalizePrompt(typeof (data as unknown as Record<string, unknown>).prompt === 'string'
      ? (data as unknown as Record<string, unknown>).prompt as string
      : '')
  );
  ...
  useEffect(() => {
    ...
    autoDetectRef.current = setTimeout(() => {
      ...
      if (tagResult) {
        ...
        setPrompt(normalizePrompt(tagResult.cleanText));
            ...
            {(['nanobanana31', 'nanobananapro'] as const).map(m => {
              ...
              return (
                <button key={m} className="nodrag" onClick={e => { e.stopPropagation(); onUpdateSettings(id, { eccoModel: m }); }}
                  style={{ flex: 1, padding: '3px 0', fontSize: 9, borderRadius: 5, border: `1px solid ${active ? '#7C3AED' : 'var(--studio-border)'}`, background: active ? '#7C3AED' : 'var(--studio-elevated)', color: active ? '#fff' : 'var(--studio-text-sec)', cursor: 'pointer' }}>
