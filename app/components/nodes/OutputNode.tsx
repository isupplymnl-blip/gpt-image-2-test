...
export default function OutputNode({ id, data }: NodeProps<OutputNodeData>) {
  const { onRegenerate, onSelectNode, onAddToLibrary, onDeleteNode, connectingFromId, onCompleteConnect, activeProvider } = useContext(StudioContext);

  const providerLabel = activeProvider === 'openai'
    ? 'OpenAI Image'
    : activeProvider === 'pudding-openai'
    ? 'Pudding OpenAI'
    : activeProvider === 'ecco'
    ? 'EccoAPI'
      ...
      <div style={{ width: imgW, height: imgH, borderRadius: 7, overflow: 'hidden', background: 'var(--studio-surface)', marginBottom: 10, position: 'relative', border: '1px solid var(--studio-border)' }}>
        {data.isLoading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            ...
            <p style={{ color: 'var(--studio-text-muted)', fontSize: 11, marginTop: 6 }}>Generating with {providerLabel}…</p>
          </div>
        )}
